...
def compute_query_response_mask(data: DataProto):
    """Compute the attention mask for the query and response part of the sequence.

    This function extracts the portion of the attention mask that corresponds to the model's query and response,
    which is used for masking computations that should only apply to query and response tokens.
    """
    responses = data.batch["query_responses"]
    response_length = responses.size(1)
    attention_mask = data.batch["query_attention_mask"]
    return attention_mask[:, -response_length:]


def compute_negative_doc_mask(data: DataProto):
    """Compute the attention mask for the negative document part of the sequence.

    This function extracts the portion of the attention mask that corresponds to the model's negative document,
    which is used for masking computations that should only apply to negative document tokens.
    """
    negative_doc = data.batch["negative_doc_responses"]
    response_length = negative_doc.size(1)
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                candidate_batch_keys = [
                    ...
                    "negative_doc_position_ids",
                ]
                batch_keys_to_pop = [
                    k
                    for k in candidate_batch_keys
                    if (batch.batch is not None and k in batch.batch.keys())
                ]

                candidate_non_tensor_batch_keys = [
                    "raw_prompt_ids",
                ...
                non_tensor_batch_keys_to_pop = [
                    k
                    for k in candidate_non_tensor_batch_keys
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if (
                        "query_responses" in batch.batch.keys()
                        and "query_attention_mask" in batch.batch.keys()
                    ):
                        batch.batch["query_response_mask"] = (
                            compute_query_response_mask(batch)
                        )
                    if (
                        "negative_doc_responses" in batch.batch.keys()
                        and "negative_doc_attention_mask" in batch.batch.keys()
                    ):
                        batch.batch["negative_doc_mask"] = compute_negative_doc_mask(
                            batch
                        )
                    ...
                    if hasattr(
                        self.reward_fn, "__class__"
                    ) and "HiddenRewardManager" in str(self.reward_fn.__class__):

                        hidden_states_dict = (
                            self.actor_rollout_wg.compute_hidden_states_fsdp(batch)
                        )
