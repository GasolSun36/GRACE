...
class SGLangRollout(BaseRollout):
    ...
    def _batch_level_generate_sequences(
        ...
        if request_sampling_params.get("n", 1) > 1 and do_sample:
            ...
            attention_mask = attention_mask.repeat_interleave(
                request_sampling_params["n"], dim=0
            )
            ...
            for key, val in non_tensor_batch.items():
                _non_tensor_batch[key] = np.repeat(
                    val, request_sampling_params["n"], axis=0
                )
        else:
    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        image_data = None
        video_data = None
        if _req.multi_modal_data is not None and isinstance(
            _req.multi_modal_data, dict
        ):
        ...
        for data_idx, (raw_prompt, multi_modal_data) in enumerate(
            ...
            for rollout_offset in range(n):
                ...
                if self.interaction_map:
                    _interaction_kwargs = prompts.non_tensor_batch[
                        "interaction_kwargs"
                    ][data_idx]
                else:
                    _interaction_kwargs = {}
