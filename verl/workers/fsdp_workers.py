...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_hidden_states_fsdp(self, data: DataProto):
        """Compute hidden states for the input data."""
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        ...
        with self.ulysses_sharding_manager:
            ...
            with adapter_ctx:
                result_dict = self.actor.compute_hidden_states_with_batching(data=data)

            result_dict_data = DataProto.from_dict(tensors=result_dict)
            output = self.ulysses_sharding_manager.postprocess_data(result_dict_data)
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage(
                "After offload actor model during compute_hidden_states", logger=logger
            )
        return output
