...
class DataParallelPPOActor(BasePPOActor):
    def __init__(
        ...
        self.config.entropy_checkpointing = True
        self.config.entropy_from_logits_with_chunking = True
    ...
    def _forward_micro_batch(
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                # Force use fused kernels when not calculating entropy to avoid OOM
                # use_fused_kernels = self.use_fused_kernels or not calculate_entropy
                ...
                # print("use_fused_kernels: ", use_fused_kernels)
                if use_fused_kernels:
    ...
    def compute_hidden_states(self, data: DataProto):
        ...
        with torch.no_grad():
            with torch.amp.autocast(
                ...
                query_output = self.actor_module(
                    input_ids=data.batch["query_input_ids"],
                    attention_mask=data.batch["query_attention_mask"],
                    output_hidden_states=True,
                    use_cache=False,
                )
                query_hidden_states = query_output.hidden_states[-1]

                neg_output = self.actor_module(
                    input_ids=data.batch["negative_doc_input_ids"],
                    attention_mask=data.batch["negative_doc_attention_mask"],
                    output_hidden_states=True,
                    use_cache=False,
                )
                neg_hidden_states = neg_output.hidden_states[-1]

        # return a DataProto object
        result_dict = {
    ...
    @GPUMemoryLogger(role="dp actor", logger=logger)
    def compute_hidden_states_with_batching(self, data: DataProto) -> dict:
        """Compute hidden states with the same batching logic as compute_log_prob

        Args:
            data (DataProto): a DataProto containing keys for main input, query input, and negative doc input

        Returns:
            dict: dictionary containing hidden states for main, query, and negative doc inputs
        """
        # set to eval
        self.actor_module.eval()
        ...
        use_gradient_checkpointing = data.meta_info.get(
            "use_gradient_checkpointing", True
        )
        sequential_processing = data.meta_info.get("sequential_processing", True)
        clear_cache_between_inputs = data.meta_info.get(
            "clear_cache_between_inputs", True
        )
        ...
        def _get_micro_batches(data: DataProto) -> Tuple[list, list | None]:
            ...
            if (
                "negative_doc_input_ids" in data.batch.keys()
                and "negative_doc_attention_mask" in data.batch.keys()
            ):
                select_keys = base_keys + [
                    "negative_doc_input_ids",
        ...
        def _forward_with_checkpointing(input_ids, attention_mask):
            """Forward pass with optional gradient checkpointing"""
            if use_gradient_checkpointing:

                def checkpoint_fn(input_ids, attention_mask):
                ...
                return torch.utils.checkpoint.checkpoint(
                    checkpoint_fn, input_ids, attention_mask, use_reentrant=False
                )
            else:
                return self.actor_module(
                    input_ids=input_ids,
        ...
        main_hidden_states_lst = []
        query_hidden_states_lst = []
        neg_hidden_states_lst = []
            ...
            with torch.no_grad():
                with torch.amp.autocast(
                    ...
                    if sequential_processing:
                        # Process each input type sequentially to reduce peak memory usage

                        # Main input hidden states
                        main_output = _forward_with_checkpointing(
                            micro_batch["input_ids"], micro_batch["attention_mask"]
                        )
                        main_hidden_states = main_output.hidden_states[-1]  # Last layer

                        main_hidden_states_lst.append(
                            main_hidden_states.clone()
                        )  # Clone to avoid memory sharing

                        if clear_cache_between_inputs:
                            del main_output
                            (
                        ...
                        query_output = _forward_with_checkpointing(
                            micro_batch["query_input_ids"],
                            micro_batch["query_attention_mask"],
                        )
                        query_hidden_states = query_output.hidden_states[-1]
                        query_hidden_states_lst.append(query_hidden_states.clone())
                        ...
                        if (
                            "negative_doc_input_ids" in micro_batch
                            and "negative_doc_attention_mask" in micro_batch
                        ):
                            neg_output = _forward_with_checkpointing(
                                micro_batch["negative_doc_input_ids"],
                                micro_batch["negative_doc_attention_mask"],
                            )
                            neg_hidden_states = neg_output.hidden_states[-1]
                            neg_hidden_states_lst.append(neg_hidden_states.clone())
                            if clear_cache_between_inputs:
                                del neg_output
                                (
                                    torch.cuda.empty_cache()
                        ...
                        main_hidden_states_lst.append(main_hidden_states)
                        query_hidden_states_lst.append(query_hidden_states)
                        if has_negative:
                            neg_hidden_states_lst.append(neg_hidden_states)

        # Concatenate all micro batch results
        main_hidden_states = torch.concat(main_hidden_states_lst, dim=0)
        query_hidden_states = torch.concat(query_hidden_states_lst, dim=0)
        neg_hidden_states = (
            torch.concat(neg_hidden_states_lst, dim=0)
            if len(neg_hidden_states_lst) > 0
            else None
        )

        # Handle dynamic batch size reordering if needed
        if use_dynamic_bsz:
            indices = list(itertools.chain.from_iterable(indices))
            assert len(indices) == main_hidden_states.size(
                0
            ), f"{len(indices)} vs. {main_hidden_states.size()}"
            revert_indices = torch.tensor(get_reverse_idx(indices), dtype=torch.long)
            main_hidden_states = main_hidden_states[revert_indices]
            query_hidden_states = query_hidden_states[revert_indices]
            if neg_hidden_states is not None:
                neg_hidden_states = neg_hidden_states[revert_indices]
        ...
        if neg_hidden_states is not None:
            result["neg_hidden_states"] = neg_hidden_states
        return result
